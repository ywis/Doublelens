...
B0834+06, we conclude that 
inclined sheet model \citep{2014MNRAS.442.3338P} can provide a precise description of the plasma lenses in the ISM between us and the pulsar, 
...
%percent accuracy.
This model can reproduce the parameters of the observed diffractive scintillation to per cent accuracy.   
Comparison with VLBI proper motion results in a
direct measure of velocity of the ionized ISM screen.  The results are consistent
...
For PSR B0834+06, we present VLBI astrometry results which provide (for the first time) a direct measurement of the distance of the pulsar.
For targets where an independent distance measurement is not available, which is the case for most
of the recycled millisecond pulsars that are the targets of precision timing observations, the degeneracy present in the lens modelling could be broken  if the pulsar resides in a binary system.
...
are already mysterious, for this pulsar, in particular, the puzzle is compounded by an offset group
...
The typical interstellar electron density is insufficient to deflect
...
perpendicular to the magenta dash dotted line, intersecting at $J$. %Short solid
                                %lines connect the model apparent
                                %image positions (point B) to the
                                %position of the first scattering
                                %(point H) for the $1$ ms group.  
The relative model pulsar-screen velocity is $185.3$ km $\rm s^{-1}$, with an angle $\phi=-3\degree.7$ east of north, is marked with an arrow from the star, point $A$, at the top of the figure.} 
\label{Doublelens}
\end{figure*}
...
We do not see the double image, placing a similar upper bound.
...
parameters (the marked points in Table \ref{table:double_lens_compare}).  The model fits the data to 2.4 percent accuracy
on each of 7 data points, and 0.1 per cent if other model fitted data just lie on the parabola, see Fig. \ref{fig:tau-fD}.  
\begin{figure}
\centering
\includegraphics[width=1.0\linewidth, angle=0]{tau-fD.pdf}
\caption{Illustration of how well the data are fit in double lens model.  Points without subscriptions are the points from the observation; points with subscriptions are the points fitted from the model, see Table \ref{table:double_lens_compare}. 
Rectangles mark the error region of point $2^\prime$, $3^\prime$, $5^\prime$ and the whole set of 1 ms group data.
We evaluate the wellness of the fitting by comparing the relative coverage of the small rectangles to the large rectangle.  
Furthermore, if we use another model, and the fitted data just lie on the parabola that is illustrated with black dash dotted lines in the figure, the wellness of the fitting is evaluated by comparing the coverage of the small rectangles over the overlapping area of the parabola and the blue dotted lines on the top and on the right.}
\label{fig:tau-fD}
\end{figure}
