...
\end{abstract}
\begin{keywords}
Pulsars: individual (B0834+06) -- scattering -- techniques: interferometric -- ISM: structure
\end{keywords}
...
only moving slowly ($\sim 21$ km $\rm s^{-1}$).
% Galactic proper motion are listed relative to LSR.
}
\label{Table:velocity}
\end{table*}
...
\hline
label&$\theta_{\parallel}$ (mas)  & $\tau_2$(ms) & $\sigma_{\rm \tau}$(ms)  & $\tau_M$(ms) & $f_D$(mHz)  &$\sigma_{f}$(mHz)      &  $f_M$(mHz)& $t_1$(d) \\ \hline
...
\caption{A 3D schematic of light path when light is double refracted. Two planes from left to right are the plane of lens 1, and the plane of lens 2. Light goes from $A$ (pulsar) to the first refracted point $H$ on lens 2 (line $HJ$, black dotted lines), and then the second refracted point $D$, the image we observe, on lens 1 (line $BD$, cyan dotted lines), and finally the observer $O$. Dash dotted lines show the light path of single deflected light path ($A-D-O$). The crosses ($A_1$ on lens 1 plane, and $A_2$ on lens 2 plane) denote the places where the un-deflected light go through the interstellar medium, marked with dotted line. Note that $A_2J\bot HJ$, and $A_1D\bot BD$.}
\label{fig:3D_image}
\end{figure*}
