...
$D_{ei} \equiv c\tau_i/\theta_i^2=D_p^2(1/D_i+1/D_{ip})$.  A third observable
effective distance is that of screen 2 using screen 1 as a lens,
$D_{e12}=D_2^2(1/D_1+1/D_{12})$, which is algebraically derivable from
the first two relations.  This means that the distance to all
subsequent lens screens can be inferred from time delays alone, and a
measurement of the effective distances does not in fact add additional
constraints. 
