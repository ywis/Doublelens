...
shore, as what is shown in Figure \ref{fig:sheetgeom}.

\begin{figure*}
\centerline{\epsfig{file=sheetgeom.eps,width=7.5in}}
\vspace{-6in}
\caption{Lensing geometry. The earth is on the left and the pulsar is on the right. A section of the (crumpled) scattering sheet is shown in the middle as the solid line. It might be a plasma underdensity or overdensity of constance thickness. The dashed line shows the unperturbed light path. The dotted line shows the path of an iamge by a fold singularity of the prjected sheet. The long-dashed line shows the position of the sheet in the absence of waves. The inclination between this sheet and the line of sight is $\alpha$, in this plot $1/8$, chosen to exaggerate the effect for visual clarity and to reduce the computational cost. In practice, we envision values closer to $10\e{-3}$.
}
\label{fig:sheetgeom}
\end{figure*}
...
the relative velocities in LSR (Local Standard of Rest). $\mu_{\alpha*}=\Delta\alpha\cos(\delta)/t$ and $\mu_{l*}=\Delta l\cos(b)/t$, for we moved the center position from $(\alpha,\delta)$ or $(l,b)$ to (0,0). The screen is
