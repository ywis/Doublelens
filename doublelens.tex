...
\author[Liu et al]{Siqi Liu$^{1,3}$\thanks{E-mail:\ sqliu@cita.utoronto.ca}, Ue-Li
  Pen$^{1,2,4}$\thanks{E-mail:\ pen@cita.utoronto.ca}, J-P Macquart$^{5}$\thanks{E-mail:\ J.Macquart@curtin.edu.au},
  Walter Brisken$^{6}$\thanks{Email:\ wbrisken@aoc.nrao.edu}, Adam Deller$^{7}$\thanks{E-mail:\ deller@astron.nl}\\
...
$^4$ Dunlap Institute for Astronomy \& Astrophysics, University of Toronto, M5S 3H4, Ontario, Canada\\
$^5$ ICRAR-Curtin University of Technology, Department of Imaging and Applied Physics, GPO Box U1978, Perth, Western Australia 6102, USA \\
...
angles \citep{2006ApJ...640L.159G}.  2. due to the odd image theorem, each caustic leads to two additional
images, more specifically, $\rm 1(original)+2\times n(additional~images~for~n~caustics)=2n+1$ (total images we observe is odd).  
In an astrophysical context, the surface could be related to
...
The secondary spectrum analysis, specifically, $P(f_\nu,f_t) = |S^{\dagger}(f_\nu,f_t)|^2$, which $\dagger$ stands for the 2-D Fourier transform, $f_\nu$ is the conjugate frequency and $f_t$ is the conjugate time.  $S(\nu,t)$ is the measurement of flux density as a function of frequency and time, which maps the information of the differential time and differential frequency of different rays of the pulsar \citep{hill2003pulsar,walker2004interpretation,cordes2006theory}.
...
The the radio frequency scaling of the arc apex positions and arc curvature in secondary spectrum are illustrated in \citet{hill2003pulsar}.  A total of $9$ apexes from the $0.4$-ms group and $5$ apexes from the $1$-ms
group, are mapped.  
This is displayed
...
We can conclude from Fig. \ref{fig:apex_pos} that this does not agree with the data.

Furthermore, in this scenario, referring to Fig. 5 in B10, the loci of the arclets on the sky should point back to the pulsar (the origin), according to \citet{2014MNRAS.442.3338P}, which also does not agree with the observation results.
...
expects two distinct sets of double lensed arcs.  We only see one
...
the odd image theorem is rarely seen to hold, which is generally thought to be
...
It might be a good supplementary study to conduct a multi-screen simulation that provides ray tracing through a large number of inclined sheets.  By adjusting the parameters, we can test the possibility of multi-refractions.  However, this is obviously beyond the scope of the current paper.
...
speed, but due to the high inclination, they will move less than one percent of this
...
amplitude of the surface waves.  This analysis indicates a high degree
...
to archival apex data of PSR B0834+06.  The data are well-fit by two
...
JPM acknowledges support through the Australian Research Council grant DP140104114.  
