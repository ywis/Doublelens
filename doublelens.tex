...
\section{Lensing}
The previous result is that for $0.4$ms apexes data, the $D_{eff}=1168\+- 23$pc, and the $1$ms data , the $D_{eff}=1121+-59$pc, which is degenerate.
To known the accurate position of the pulsar, we need to break this degeneracy.

this time, when we deal with the archival data.I need to define how I relate tau-D; fD-velovity.; 

from tau, RA and dec
we obtained the result that the positions of the pulsar is 
$0.4$ms ,$D_{eff}=1017+-2.8$pc,
$1$ms, $D_{eff}=1121\+-64$pc
where the degeneracy is broken.

This is how the result is obtained. Known the distance of the pulsar is $640$pc,
$0.4$ms pile, $D_s$ is equivalent to $392.82$pc. 
Theta is obtained from tau. Known the axis( -25.2deg) we get the ra and dec.

$1$ms pile, the $D_s$ is equivalent to $422.5$pc.
We fit a line to the $1$ms observation positions. Lying on the fitted line, point 5 is obtained with equal theta_para as the farthest point in the 0.4ms pile. Knowing the fD of this point, we get vA5. Similarly, we obtained the v_parallel from the 0.4ms data. Thus, the total velocity of the pulsar is determined.
Knowing tau, we can get the theta of the pulsar. Knowing the line and the differential frequency, we can get the ra and dec of the pulsar.

But accordign to Fermat's law, the distributions of the pile of 1ms points are not reasonable. If they are being reflected by one interstellar medius lens, they should be distributed along a line like the 0.4ms pile, just lie in the direction of the pulsar and point J in \ref{Doublelens}
...
We made a plot of the reflected velocity in the direction that is transverse to the first lens plane in \figure{vtrans}.
...
\begin{figure*}
\centering
\includegraphics[width=1.0\textwidth,angle=0]{V_trans_theta_para.png}
\caption{plot the relation of vtrans vs thetapara}
\label{vtrans}
\end{figure*}
