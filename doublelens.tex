...
\caption{Reflection of lights on surface waves.  At grazing angles,
 ...
 center in the absence of waves.}
\label{fig:water_reflection}
\end{figure*}
...
We select the equivalent apexes from four sub-bands. To match the same apexes in different sub-bands, we scale the differential frequency in different sub-bands to $322.5$ MHz, by $f_D (322.5/f_{\rm band})$ MHz. A total of $9$ apexes from the $0.4$ ms group and $5$ apexes from the $1$ ms
...
\ref{table:apex}. The $f_D$, $\tau$, $\Delta\alpha$ and $\Delta\delta$
are the mean values of $n$ sub-bands, where $n$ is the number of the
sub-bands ($n=3$ for points 4 to 6, and 4 for the rest points), listed
...
\hline
label & $\theta_{\parallel}$(mas) & $f_D$(mHz) & $\tau$(ms)  & $\Delta\alpha$(mas) & $\Delta\delta$(mas) & $t_0$(d)\\
\hline
...
\begin{equation}
\sigma^2_{\rm \tau, f_D,\Delta\alpha, \Delta\delta} = \frac{1}{n}\sum^{n}_{i=1}\frac{(x_i-\bar{x})^2}{n-1},
\end{equation}
...
In general, $D_{\rm e}={D_{\rm p} D_{\rm s}}/({D_{\rm p} - D_{\rm
    s}})$ for a screen at $D_{\rm s}$.  Fig. \ref{fig:Singledegeneracy} shows two sets of $D_p$, $D_s$ with common $D_e$.

\begin{figure*}
\centering
%\epsscale{1.0}
\hspace*{-1in}\includegraphics[width=9in]{single_degeneracy.pdf}
...
\caption{Angular positions of $0.4$ ms and $1$ ms group data in one lens model and double lens model. The axes represent the relative distance to the un-refracted pulsar in Right Ascension (calculated by $x= \Delta \alpha \cos(\delta)D$ and $D$ represent the distance of the object to the observer) and declination (calculated by $y=\Delta \delta D$, with $D$ defined previously) directions, on a 2D plane that is transverse to the line of sight. %The screen distances are $388$ pc and $420$ pc for left (lens 1) and right (lens 2) groups respectively. 
On the left side, the points marked with letter $D$, are the derived
positions from the time delays of $0.4$ ms group in the  one lens
model. $389$ pc away from the observer, the dashed line aligns the
...
refraction images in double lens model. The unobservable points
...
dot-dash line passing through $J$ is the inferred geometry of the second
lens.  Its thicker portion has formed a full caustic, while the
thinner portion are sub critical.
The dot 
dash lines, constructed perpendicular to the $AD$ scattering axis,
denote the caustics of lens 1. The dotted line on the top right is
perpendicular to the solid line, intersecting at $J$. %Short solid
                                %lines connect the model apparent
                                %image positions (point B) to the
                                %position of the first scattering
                                %(point H) for the $1$ ms group.  
...
Apexes 1'--5' share a similar 1ms time delay, suggesting they are lensed by
a common structure.
...
We first calculate the position of $J$. We  estimate the
...
marked in Fig. \ref{Doublelens}. The long solid line on the right side
...
For $v_{\parallel}$, we derive the velocity $172.4 \pm 2.4$ km $\rm s^{-1}$ from $f_D$ of point $1$ in $0.4$ ms group.  The direct observable is the time to crossing of each caustic, denoted $t_0$ in Table \ref{table:apex}. 
...
$\epsilon=-3\degree.7$, which is west of north. This resents the
...
\hline
label&$\theta_{\parallel}$ (mas)  & $\tau_2$(ms) & $\sigma_{\rm \tau}$(ms)  & $\tau_{\rm M}$(ms) & $f_D$(mHz)  &$\sigma_{f}$(mHz)      &  $f_{\rm M}$(mHz)& $t_1$(d) \\ \hline
...
near double lens image ', and thus only intersections with the closer
...
5'; and $78$ d 
before 2005 November 12, it overlayed on point 1'. The
...
\caption{Deflection angle $\beta=\pi-\protect\angle$AHB on lens 2.  Point $J$ denotes the expected position to form a single refraction image, which was not
  ...
  10 per cents bandwidth, with a 20 per cents change in refractive strength.  We
...
\end{figure*}
A 3D schematic light path is displayed in Fig. \ref{fig:3D_image}.
\begin{figure*}
%\centering
...
\caption{A 3D schematic of light path when light is double
  ...
  and the plane of lens 2.  The dot-dashed lines represent the
  projected line-like fold caustics. 
Light goes from $A$ (pulsar) to the first
  refracted point $H$ on lens 2 (line $HJ$, black dotted lines), and
  then the second refracted point $D$, the image we observe, on lens 1
  (line $BD$, cyan dotted lines), and finally the observer $O$. Dash
