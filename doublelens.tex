...
overviews the inclined sheet lensing model, and its application to data.
...
\label{fig:sheetgeom}
\end{figure*}
The enlarged structure of the light path near point D ripple is illustrated in Fig. \ref{fig:refractionD}.
\begin{figure*}
%\vspace{-0.8in}
\includegraphics[width=1.0\textwidth, angle=0]{refraction.pdf}
\caption{The refraction of the light near point D.  The black lines are the light paths.  The shaded region mark the interstellar medium.  The angles obey the Snell's Law $\sin(\alpha_1)/\sin(\alpha_2)=\sin(\alpha_4)/\sin(\alpha_3)=n_{\rm ISM}/n_{\rm void}.$
}
...
5& $-13.68$ & $-21.6(6)$        & 0.236(2)    & 5.1(4)  & $-12.7(5)$      & $-81.8$                                \\
...
the relative velocities in LSR (Local Standard of Rest). $\mu_{\alpha*}=\Delta\alpha\cos(\delta)/t$ and $\mu_{l*}=\Delta l\cos(b)/t$, for we move the center position from $(\Delta\alpha,\Delta\delta)$ or $(l,b)$ to (0,0).  $v_{l*}$ and $v_b$ are the linear velocities relative to the LSR.  The screen is
