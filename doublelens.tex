...
delay rate, RA and dec, one for each of four $8$ MHz wide sub-bands, centering $314.5$ MHz, $322.5$ MHz, $328.5$ MHz and $334.5$ MHz.  We mapped
...
The way we used to calculate error of time delay $\tau$ and differential frequency $f_D$ is by the following equation:
\begin{align*}
...
By using the refraction law, we solve the solutions in double lens model.And the solved positions are plotted in Figure \ref{Doublelens}, and respective time delays and differential frequencies are listed in Table \ref{table:double_lens_compare}. 
\begin{equation}
\sigma = \sqrt{\sigma_1^2+\sigma_2^2}
\end{equation}

Figure \ref{first_reflect} and Figure \ref{second_reflect} are examples of how light are being refracted on the first lens plane and the second lens plane, with $\theta_{\parallel}$ equal to $-17.44$ mas of the secondary lens plane position.
