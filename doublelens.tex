...
\end{aligned}
\end{equation}
where k is denoted as the slope in Figure\ref{thetatau}, $\sigma_{ki}$ as the error region of the slope. $\sigma_{ki}=\sigma_{\theta i}/\sqrt{\tau}$, $\sigma_{\theta}=\sqrt{\sigma_{\Delta\alpha}^2+\sigma_{\Delta\delta}^2}$, and $n=9$ for $1$ ms group, $n=4$ for $0.4$ ms group. This results in
...
images.  This scenario predicts that at frequencies just above $300$
...
\ref{vtrans}.  We denote $t_j$ as the time period for the reflected image on lens 2 of the position of the pulsar moves from point $j$ to point J, where $j$ is for points $1,2,3,4,5$. From our calculation, $37$ days before the lensed pulsar
...
\begin{equation}
\begin{aligned}
(\frac{\sigma_{\tau\rm tot}}{\tau_2})^2 = (\frac{\sigma_{\tau1}}{\tau_1})^2+(\frac{\sigma_{\tau2}}{\tau_2})^2 + (\frac{\sigma_{\tau22}}{\tau_{22}})^2,
%\frac{\sigma_{\rm tot}}{\tau_2} = (\frac{\sigma_{\rm 1sample}}{\tau_1})^2+(\frac{\sigma_{\rm 2sample}}{\tau_2})^2,
\end{aligned}
\end{equation}
where $\tau_1$ and $\sigma_{\tau1}$ represent the time delay and its error from the $0.4$ ms group on lens $1$, $\tau_2$ and $\sigma_{\tau2}$ represent the time delay and its error from respective $1$ ms group on lens 2, and $\tau_22$ is the $\tau_2$ for the point in the second row in Table \ref{table:double_lens_compare} and $\sigma_{\tau22}$ is its error.
...
\begin{equation}
\begin{aligned}
(\frac{\sigma_{f_D\rm tot}}{f_D})^2=(\frac{\sigma_{f_D}}{f_D})^2+(\frac{\sigma_{f_D2}}{f_{D2}})^2
\end{aligned}
\end{equation}
where $f_{D2}$ and $\sigma_{f_D2}$ are the differential frequency and its error of the point in the second row in Table \ref{table:double_lens_compare}.

\subsubsection{Comparing calculated result in double lens model and observation}
Comparing $\tau$, we calculated time delay $\tau_C$ for these five points, and list the results in Table \ref{table:double_lens_compare}. For point 2 and 5, they fit perfectly because these are the two points that we use these to calculate the position of J; for the rest three points, all of the calculated results are still within $3-\sigma$ region of the observation time delays.
...
With this velocity of the pulsar, we calculate the differential frequency $f_{DC}$ of point 1,3,4 and 5. Results are listed in Table \ref{table:double_lens_compare}. The calculated results all lie in the $3-\sigma$ region of the observation data. 

The reduced ${\chi}^2$ for time delay $\tau$ is $6.1$ for $3$ degrees of freedom, and the reduced ${\chi}^2$ for differential frequency $f_D$ is $3.6$ for $4$ degrees of freedom.
...
\hline
$\theta_{1\parallel}$ (mas)  & $\tau_2$(ms) & $\sigma_{\rm \tau tot}$(ms)  & $\tau_C$(ms) & $f_D$(mHz)  &$\sigma_{f_D \rm tot}$(mHz)      &  $f_{DC}$(mHz)& $t_j$(day) \\ \hline
-13.82  &    0.9495     &0.0094    & 0.9550       & -35.1     &0.81    & -37.22          & -37\\ 
-14.59  &   0.9763    &0.00088   & 0.9763*       & -38.3     &0.64    & -38.31$\dagger$ & -40\\ 
-16.24  &   1.005    &0.011   & 1.0272          & -40.17    &0.87   & -40.64          & -46\\ 
-16.52  &    1.0370     &0.0034    & 1.0363       & -41.27    &0.88   & -41.04          & -47\\ 
-17.39  &  1.0663     &0.0050    & 1.0663*        & -43.08    &0.84   & -42.27           & -50\\ \hline
\end{tabular}
}
\caption{Comparison of time delay $\tau$ and the differential frequency $f_D$ of the observation and the calculated result in double lens model. $\theta_{1\parallel}$ is the angle of the $0.4$ ms group with the component in the axis defined by $\gamma$. 
