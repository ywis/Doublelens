...
%\tablewidth{1in}%\columnwidth}
\tablecaption{Fitted and derived astrometric parameters for PSR B0834+06.}
\tablewidth{0pt}
\startdata
Reference right ascension (J2000)\tablenotemark{a}
							&  08:37:5.644606(9)\\
Reference declination (J2000)\tablenotemark{a}
							&  06:10:15.4047(1)\\
...
\enddata
\tablenotetext{a}{The errors quoted here are from the astrometric fit only and do not include the $\sim$1 mas position uncertainty transferred from the in--beam calibrator's absolute position.}
\label{tab:vlbi}
\end{deluxetable}
...
For $v_{\parallel}$, we derive the velocity $175.6 \pm 2.7$ km/s from $f_D$ of point 1 in $0.4$ ms group.  The direct observable is the time to crossing of each caustic, denoted $t_0$ in Table \ref{table:apex}. 
...
With $v_{\bot}$ to be $66.8\pm 2.8$ km/s, we find $v_{\rm tot}=187.9 \pm 2.8$ km/s, with an angle $\epsilon=4\degree.34$ west of north. This velocity represents the pulsar's relative velocity to the earth, subtracting the screen's relative velocity to the earth. The model derived and observation velocities (heliocentric and in Local Standard of Rest) are listed in Table \ref{Table:velocity}. The direction of the model velocity is marked on the top of the star in Figure \ref{Doublelens}.
