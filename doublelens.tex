...
\begin{align*}
\sigma_{\rm sample} = \sqrt{\frac{\sum^n_{i=1}(x_i-\bar{x})^2}{n(n-1)}},
\end{align*}
...
\begin{align*}
\sigma_{\rm population} = \sqrt{\frac{1}{\sum^n_{i=1}{\frac{1}{{ \rm err_i}^2}}}},
\end{align*}
where the i ranges from 1 to 4, for four sub-bands, and $\rm err_i$ is the errorbar in each sample. They are marked with circles in Figure \ref{Doublelens}.
...
\begin{align*}
\sigma_{\rm tot} = \tau_2 \sqrt{(\frac{\sigma_{\rm 1sample}}{\tau_1})^2+(\frac{\sigma_{\rm 2sample}}{\tau_1})^2},
\end{align*}
where $\sigma_{\rm 1sample}$ represents the sample error from the $0.4$ ms group on lens 1, and $\sigma_{\rm 2sample}$ represents the sample error from respective $1$ ms group on lens 2, $\tau_1$ is the time delay from $0.4$ ms group, and $\tau_2$ is the time delay from respective $1$ ms group.
...
-14.63  & -14.69 &  0.9763    &0.0015   & 0.9763       & -38.3 &1.0   & -38.31 \\ 
