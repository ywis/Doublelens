...
Using archval data we demonstrate in this paper that the lensing screen
...
\vspace{-7.5in}
\caption{Lensing geometry (reproduced from \citet{2014MNRAS.442.3338P}). See text for details.
}
\label{fig:sheetgeom}
\end{figure*}
...
\caption{The water reflections of the buildings on the shore. The water reflection of a single light source lie in a straight line. Because the light sources are not close to the edge, we cannot see a separation to the edge, due to the critical angle of reflection. Photo taken by Michael Williams.}
...
In general, $D_{\rm e}={D_{\rm p} D_{\rm s}}/({D_{\rm p} - D_{\rm
    s}})$ for a screen at $D_{\rm s}$. Figure \ref{fig:Single_degeneracy} shows two sets of $D_p$, $D_s$ with one $D_e$.

\begin{figure*}
\centering
%\epsscale{1.0}
\includegraphics[width=1.0\linewidth, angle=0]{single_degeneracy.png}
\caption{A single reflected light path showing the distance degeneracy. O denotes the observer; A and A' are denote the positions of the pulsar; D and D' denote the positions of the reflected images on the ISM. When $D_p=620~$pc, the light path is A-D-O; when $D_p=1240~$pc, the light path is A'-D'-O. They share the same time delay, angular offset and effective distance. }
...
\caption{The geometry of the distance degeneracy of double reflected light path. O denotes the observer; A and A' denote the positions of the pulsar; H and H' denote the positions for the first reflected images; B and B' denote the positions of the second reflected images. When $D_p=620~$pc, the light path is A-H-B-O; when $D_p=1240~$pc, the light path is A'-H'-B'-O. H and H' share the same angular offset, and effective distance. B and B' share the same angular offset and effective distance.}
...
\caption{A 3D version of light path when light is double reflected. Four planes are the plane of the pulsar, plane of lens 1, plane of lens 2 and the observer plane. Light goes from A (pulsar) to the first reflected image H on lens 1, and then the second reflected image H on lens 2, and finally the observer O.}
\label{fig:3D_image}
\end{figure*}
...
Peter Goldreich and M. van Kerkwijk. We thanks Michael Williams for taking the water reflection photo.  Siqi Liu thank the useful discussion with Robert Main and JD Emberson on improving the expression of the content. 
