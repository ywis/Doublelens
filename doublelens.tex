...
\vspace{-7.5in}
\caption{Lensing geometry (reproduced from \citet{2014MNRAS.442.3338P} Figure 1). See text for details.
}
\label{fig:sheetgeom}
\end{figure*}
...
\caption{A single refracted light path showing the distance degeneracy. O denotes the observer; A and A' are denote the positions of the pulsar; D and D' denote the positions of the reflected images on the interstellar medium. When $D_p=620~$pc, the light path is A-D-O, which is marked with solid lines; when $D_p=1240~$pc, the light path is A'-D'-O, marked with dot dashed lines. They share the same time delay, angular offset and effective distance. }
