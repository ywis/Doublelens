...
delay rate (differential frequency $f_D$), Right Ascension $\alpha$ and declination $\delta$. Data of each apex are collected from four dual circular polarization $8$ MHz wide sub-bands spanning the frequency range $310.5-342.5$ MHz. We divide the apex data into two groups according to the time delay and map
a total of $9$ apexes from the $0.4$ ms group, and $5$ apexes from the $1$ ms
group. This results in an estimation
for the average value in $f=322.5$ MHz and standard deviation among four sub-bands. For $0.4$ ms region, they are listed in Table
\ref{table:apex}; for $1$ ms, they are listed in Table \ref{table:apex2}. The time is calculated with $2{\tau}f/{f_{D}}$,
...
The method we use to calculate the error of time delay $\tau$ and differential frequency $f_D$ is by following equation:
\begin{align*}
{\sigma_{\rm \tau, f_D}}^2 = \frac{\sum\limits^n_{i=1}(x_i-\bar{x})^2}{n(n-1)},
\end{align*}
where i ranges from 1 to $5$ in $0.4$ ms group, and i ranges from $1$ to $9$ in $1$ ms group. This is also how we calculate the sample error of $\alpha$ and $\delta$, marked with errorbars in Figure \ref{Doublelens}.
The way we use to calculate the population error of $\alpha$ and $\delta$, is defined by following equation:
\begin{align*}
{\sigma_{\rm \alpha, \delta}}^2 = \frac{1}{\sum\limits^n_{i=1}{\frac{1}{{ \rm \sigma_i}^2}}},
\end{align*}
where the i ranges from 1 to 4, for four sub-bands, and $\rm \sigma_i$ is the errorbar in each sample. They are listed in Table\ref{table:apex} and marked with circles in Figure \ref{Doublelens}.
...
\citet{2014MNRAS.442.3338P}.  In this model, the lensing is due to projected fold caustics of a thin sheet closely aligned to the line of sight. 
...
Take $D_{1O}=392.8$ pc, combined with the VLBI measured distance of the pulsar $640$ pc, the distance of lens 1, where $0.4$ ms scintillation points are refracted, is equal to $392.8$ pc. From the fitted position of VLBI measurement, we already know they lie on an straight line, which has an positive angle of $\gamma$ $-25.2\degree$ (east of the declination axis). We use this axis to define $\theta_{\parallel}$ and define $\theta_{\bot}$ by a $90\degree$ clockwise rotation from it. Then the positions of the $0.4$ ms group are calculated:
\begin{align*}
...
\begin{align*}
\tau(t=0{\rm s})-\tau(t=6500~{\rm s},v_{\parallel})&=f_{\rm D6}/f\cdot{\rm dt},\\
\tau(t=0 {\rm s}) -\tau(t=6500~{\rm s}, v_{\rm A5}) & = f_{\rm D5} /f\cdot{\rm dt},
\end{align*}
...
v_{\rm tot}\cdot {\rm sin[ tan^{-1}(\alpha,\delta)-\epsilon]} &=v_{\rm A4},\\
...
\begin{align*}
\frac{\sigma_{\rm tot}}{\tau_2} = (\frac{\sigma_{\tau1}}{\tau_1})^2+(\frac{\sigma_{\tau2}}{\tau_2})^2,
...
\\ \hline
\end{tabular}
\caption{$0.4$ ms observation data and calculated data.
% The upper part of the table list the $0.4$ ms group data, while the $1$ ms group lie in the lower part of the table. 
Observation data include the differential frequency $f_D$, time delay $\tau_1$ from scintillation measurement; $\alpha$ and $\delta$ from the VLBI measurement. The method of how to calculate the error of time delay, differential frequency and the last column time is mentioned in Section \ref{21}. $\theta_{1\parallel}$, the angle of the $0.4$ ms group with the component in the axis defined by $\gamma$, are derived from the $\theta-\tau$ relation plotted in Figure \ref{thetatau}.}
\label{table:apex}
\end{table*}
...
\begin{tabular}{llllllllll}
\hline
$f_D$(mHz) & $\sigma_{f_D}$(mHz) & $\tau_2$(ms) & $\sigma_{\tau2}$(ms) & $\alpha$(mas) & $\sigma_{\alpha}$(mas) & $\delta$(mas) & $\sigma_{\delta}$(mas) & time(day)\\
\hline
        -35.06                           & 0.52                               & 0.9495               & 0.0016                              & -15.23  & 0.69                                     & -21.06   & 0.70   &202                                   \\

 -38.31                           & 0.64                               & 0.97633              & 0.00088                             & -15.02  & 0.48                                    & -20.74   & 0.38  &190                                    \\

 -40.17                           & 0.55                               & 1.0045              & 0.0079                             & -14.14  & 0.66                                    & -22.27   & 0.62  &187                                   \\

 -41.27                           & 0.54                               & 1.0370              & 0.0032                              & -11.28  & 0.93                                     & -19.2   & 1.1   &188                                   \\

 -43.08                           & 0.44                               & 1.0663              & 0.0050                              & -8.4   & 1.7                                      & -24.1   & 1.4   &185   \\
 \hline                                 
\end{tabular}
\caption{$1$ ms observation data and calculated data. 
%The upper part of the table list the $0.4$ ms group data, while the $1$ ms group lie in the lower part of the table. 
Observation data include the differential frequency $f_D$, time delay $\tau_2$ from scintillation measurement; $\alpha$ and $\delta$ from the VLBI measurement. The method of how to calculate the error of time delay, differential frequency and the last column time is mentioned in Section \ref{21}.}
